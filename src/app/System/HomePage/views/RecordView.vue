...
import { type ISystemRecordItem, Record } from '@/api/system/actions'
import ReadonlyList from '@/components/List/ReadonlyList.vue'
import { useMessage } from '@/components/Message'
import { h, onMounted, reactive, ref } from 'vue'
import { createCustomModal } from '@/components/Modal'
import RecordEditVue from './_EditView/RecordEdit.vue'
import { date2String } from '@/utils/_'

type TItem = ISystemRecordItem
const Actions = Record

const colMap = [
  { name: 'ID', key: 'id' },
  { name: '诊断ID (diagnosis_id)', key: 'diagnosis_id' },
  {
    name: '记录时间 (record_date)',
    key: 'record_date',
    func: (value: any) => date2String(value)
  },
  {
    name: '记录图像 (image_source)',
    key: 'image_source'
  },
  { name: 'X%', key: 'main_vessel_ratio_x' },
  { name: 'Y%', key: 'general_vessel_ratio_y' },
  { name: 'Z%', key: 'tiny_vessel_ratio_z' },
  { name: '缺血区域V', key: 'ischemic_area_v' },
  { name: '血管密度P', key: 'vessel_density_p' }
]

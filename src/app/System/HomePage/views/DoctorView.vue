...
import { type ISystemDoctorItem, Doctor } from '@/api/system/actions'
import ReadonlyList from '@/components/List/ReadonlyList.vue'
import { useMessage } from '@/components/Message'
import { h, onMounted, reactive, ref } from 'vue'
import { createCustomModal } from '@/components/Modal'
import DoctorEditVue from './_EditView/DoctorEdit.vue'
import { stringEllipsis } from '@/utils/_'

type TItem = ISystemDoctorItem
const Actions = Doctor

const colMap = [
  { name: 'ID', key: 'id' },
  { name: '姓名 (name)', key: 'name' },
  { name: '用户名 (username)', key: 'username' },
  { name: '密码hash (password)', key: 'password', func: (value: any) => stringEllipsis(value, 12) },
  { name: '是否激活 (active)', key: 'active' }
]
